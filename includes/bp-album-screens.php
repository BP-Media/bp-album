...
function bp_album_screen_edit_content() {
	global $bp,$pictures_template;
	if (bp_album_has_pictures()  && ( bp_is_my_profile() || is_site_admin() ) ) :  bp_album_the_picture();
...
<div id="message" class="info">
	<p><?php echo bp_word_or_name( __( "This url is not valid.", 'bp-album' ), __( "Either this url is not valid or you can't edit this picture.", 'bp-album' ),false,false ) ?></p>
</div>
	<?php endif;
}
...
function bp_album_screen_upload() {
	...
	if( ! bp_is_my_profile() ) {
		if ( !empty($bp->loggedin_user->domain) ) {
			bp_core_redirect( $bp->loggedin_user->domain . $bp->album->slug . '/'. $bp->album->upload_slug .'/');
		} else {
			auth_redirect();
		}
	}
