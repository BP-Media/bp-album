...
if ( ! $lib_versions->allOK() ) {
  ...
  function bp_album_install() {
    ...
    if ( ! get_site_option( 'bp_album_max_upload_size' ) ) {
      update_site_option( 'bp_album_max_upload_size', 6 );
    } // 6mb
   ...
   * item and all the sublevel nav items to the navigation array. This is then
  ...
  function bp_album_setup_nav() {
    ...
    $album_link       = ( isset( $bp->displayed_user->id ) ? $bp->displayed_user->domain : $bp->loggedin_user->domain ) . $bp->album->slug . '/';
    $album_link_title = isset( $bp->displayed_user->id ) ? bp_word_or_name( __( "My pictures", 'bp-album' ), __( "%s's pictures", 'bp-album' ), false, false ) : __( "My pictures", 'bp-album' );
...
function bp_album_load_template_filter( $found_template, $templates ) {
  ...
  if ( empty( $found_template ) ) {
    ...
    add_action( 'bp_template_content', 'bpa_template_content_action' );
  }
...
function bpa_template_content_action() {
  global $bp;
...
function bp_album_limits_info() {
  ...
  $results = bp_album_get_picture_count( [
    'owner_id'      => $owner_id,
    'privacy'       => 'all',
    'priv_override' => true,
    'groupby'       => 'privacy',
...
function bp_album_delete_by_user_id( $user_id, $remove_files = true ) {
  ...
  if ( $remove_files ) {
    $pics = BP_Album_Picture::query_pictures( [
      'owner_type' => 'user',
      'owner_id'   => $user_id,
      'per_page'   => false,
      'id'         => false,
  ...
  if ( function_exists( 'bp_activity_delete' ) ) {
    ...
    bp_activity_delete( [ 'component' => $bp->album->id, 'user_id' => $user_id ] );
  }
...
function bp_album_record_activity( $pic_data ) {
  ...
  $id = bp_activity_get_activity_id( [ 'component' => $bp->album->id, 'item_id' => $pic_data->id ] );
  ...
  return bp_activity_add( [
    'id'            => $id,
    'user_id'       => $pic_data->owner_id,
    'action'        => $action,
    'content'       => $content,
    'primary_link'  => $primary_link,
    'component'     => $bp->album->id,
    'type'          => $type,
    'item_id'       => $item_id,
    'recorded_time' => $pic_data->date_uploaded,
...
function bp_album_delete_activity( $user_id ) {
  ...
  return bp_activity_delete( [ 'component' => $bp->album->id, 'item_id' => $user_id ] );
}
...
function bp_album_dump( &$var, $info = false ) {
  ...
  echo "<pre style='margin: 0 0 10px 0; display: block; background: white; color: black; font-family: Verdana,serif; border: 1px solid #cccccc; padding: 5px; font-size: 10px; line-height: 13px;'>";
  if ( $info ) {
...
function bp_album_rebuild_activity() {
  ...
  $sql     = $wpdb->prepare( "SELECT * FROM {$bp->album->table_name} WHERE privacy = %d", 0 );
  ...
  foreach ( $results as $pic_data ) {
    ...
    $sql      = $wpdb->prepare( "SELECT id FROM {$bp->activity->table_name} WHERE component = %s AND item_id = %d", $bp->album->id, $pic_data->id );
  ...
  $sql              = $wpdb->prepare( "SELECT date_recorded FROM {$bp->activity->table_name} ORDER BY date_recorded ASC LIMIT %d", 1 );
  ...
  $sql     = $wpdb->prepare( "SELECT id FROM {$bp->activity->table_name} WHERE component = %s AND secondary_item_id = %d", $bp->album->id, 999 );
  $results = $wpdb->get_results( $sql );
  ...
  foreach ( $results as $post ) {
    ...
    $sql = $wpdb->prepare( "UPDATE {$bp->activity->table_name} SET date_recorded = %s WHERE id = %d", $new_date, $post->id );
    $wpdb->query( $sql );
  }
  unset( $results );
  unset( $post );
...
function bp_album_undo_rebuild_activity() {
  ...
  return bp_activity_delete( [ 'component' => $bp->album->id, 'secondary_item_id' => 999 ] );
}
