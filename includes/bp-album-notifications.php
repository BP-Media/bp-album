...
function bp_album_send_high_five_notification( $to_user_id, $from_user_id ) {
	global $bp;
	...
	$sender_highfive_link = site_url( BP_MEMBERS_SLUG . '/' . $sender_ud->user_login . '/' . $bp->album->slug . '/screen-one' );
	...
	$subject = '[' . get_blog_option( 1, 'blogname' ) . '] ' . sprintf( __( '%s high-fived you!', 'bp-album' ), stripslashes($sender_name) );
...
---------------------
', 'bp-album' ), $sender_name, $sender_name, $sender_profile_link, $sender_name, $sender_highfive_link );
...
add_action( 'bp_album_send_high_five', 'bp_album_send_high_five_notification', 10, 2 );
