...
function bp_album_screen_edit_content() {
  ...
  if ( bp_album_has_pictures() ) : bp_album_the_picture();
    ...
    ?>
      ...
      <form method="post" enctype="multipart/form-data" name="bp-album-edit-form" id="bp-album-edit-form"
            class="standard-form">
        ...
        <?php if ( bp_is_active( 'activity' ) && $bp->album->bp_album_enable_comments ): ?>
            <p>
                ...
                <label><input type="radio" name="enable_comments" value="1"
                              checked="checked"/><?php _e( 'Enable', 'bp-album' ) ?></label>
                <label><input type="radio" name="enable_comments" value="0"/><?php _e( 'Disable', 'bp-album' ) ?>
                </label>
...
function bp_album_screen_upload_content() {
  ...
  if ( $limit_info['all']['enabled'] && ( $limit_info['all']['remaining'] > 0 ) ):?>

      <h4><?php _e( 'Upload new picture', 'bp-album' ) ?></h4>

      <form method="post" enctype="multipart/form-data" name="bp-album-upload-form" id="bp-album-upload-form"
            class="standard-form">
...
function bp_album_action_upload() {
  ...
  if ( $bp->current_component == $bp->album->slug && $bp->album->upload_slug == $bp->current_action && isset( $_POST['submit'] ) ) {
    ...
    if ( ! isset( $_POST['privacy'] ) ) {
      ...
      } elseif ( $pic_limit !== false && ( $pic_limit === 0 || $pic_limit <= bp_album_get_picture_count( [ 'privacy' => $priv_lvl ] ) ) ) {
    ...
    if ( isset( $_FILES['file'] ) ) {
      ...
      if ( $_FILES['file']['error'] ) {
        ...
        /* translators: 1: error message */
          __( 'Your upload failed, please try again. Error was: %s', 'bp-album' ),
          $uploadErrors[ $_FILES['file']['error'] ] );
        $error_flag         = true;
      } elseif ( ( $_FILES['file']['size'] / ( 1024 * 1024 ) ) > $bp->album->bp_album_max_upload_size ) {

        $feedback_message[] = sprintf(
        /* translators: 1: maximum file size in MiB (1024 * 1024( */
          __( 'The picture you tried to upload was too big. Please upload a file less than %1$d MiB', 'bp-album' ),
          $bp->album->bp_album_max_upload_size );
        $error_flag         = true;
      }
    ...
    if ( ! $error_flag ) {
      ...
      $pic_org = wp_handle_upload( $_FILES['file'], [ 'action' => 'picture_upload' ] );
    ...
    if ( ! $error_flag ) {
      ...
      if ( $pic_org_size <= $bp->album->bp_album_thumb_size ) {
        ...
        $pic_editor = wp_get_image_editor( $pic_mid_path );
        if ( is_wp_error( $pic_editor ) ) {
          $feedback_message[] = __( 'Image resizing failure', 'bp-album' ) . esc_html( implode( '; ', $pic_editor->get_error_messages() ) );
          $error_flag         = true;
        }
        $pic_resized = $pic_editor->resize( $bp->album->bp_album_thumb_size, $bp->album->bp_album_thumb_size, true );
        if ( is_wp_error( $pic_resized ) ) {
          $feedback_message[] = __( 'Image resizing failure:', 'bp-album' ) . esc_html( implode( '; ', $pic_resized->get_error_messages() ) );
          $error_flag         = true;
        }
        $pic_thumb = $pic_editor->generate_filename();
        $pic_saved = $pic_editor->save( $pic_thumb );
        if ( is_wp_error( $pic_saved ) ) {
          $feedback_message[] = __( 'Image resizing failure:', 'bp-album' ) . esc_html( implode( '; ', $pic_saved->get_error_messages() ) );
          $error_flag         = true;
        }
...
function bp_album_upload_dir() {
  ...
  return apply_filters( 'bp_album_upload_dir', [
    'path'    => $dir,
    'url'     => $url,
    'subdir'  => $subdir,
    'basedir' => $bdir,
  ...
  if ( $bp->current_component == $bp->album->slug && $bp->album->single_slug == $bp->current_action && $pictures_template->picture_count && isset( $bp->action_variables[1] ) && $bp->album->edit_slug == $bp->action_variables[1] && isset( $_POST['submit'] ) ) {
    ...
    if ( ! isset( $_POST['privacy'] ) ) {
      ...
      if ( $pic_limit === null ) {
        $error_flag         = true;
        $feedback_message[] = __( 'Privacy option is not correct.', 'bp-album' );
      } elseif ( $pic_limit !== false && $priv_lvl !== $pictures_template->pictures[0]->privacy && ( $pic_limit === 0 || $pic_limit <= bp_album_get_picture_count( [ 'privacy' => $priv_lvl ] ) ) ) {
        $error_flag = true;
        switch ( $priv_lvl ) {
          case 0 :
...
function bp_album_action_delete() {
  ...
  if ( $bp->current_component == $bp->album->slug && $bp->album->single_slug == $bp->current_action && $pictures_template->picture_count && isset( $bp->action_variables[1] ) && $bp->album->delete_slug == $bp->action_variables[1] ) {
    ...
    if ( ! $pictures_template->picture_count ) {
      ...
      if ( ! bp_is_my_profile() && ! current_user_can( 'delete_others_posts' ) ) {
