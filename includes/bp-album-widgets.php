...
class BP_Example_Widget extends WP_Widget {
	...
	function form( $instance ) {
		...
		<p><label for="bp-album-widget-per-page"><?php _e( 'Number of Items Per Page:', 'bp-album' ); ?> <input class="widefat" id="<?php echo $this->get_field_id( 'per_page' ); ?>" name="<?php echo $this->get_field_name( 'per_page' ); ?>" type="text" value="<?php echo attribute_escape( $per_page ); ?>" style="width: 30%" /></label></p>
		<p><label for="bp-album-widget-max"><?php _e( 'Max items to show:', 'bp-album' ); ?> <input class="widefat" id="<?php echo $this->get_field_id( 'max_items' ); ?>" name="<?php echo $this->get_field_name( 'max_items' ); ?>" type="text" value="<?php echo attribute_escape( $max_items ); ?>" style="width: 30%" /></label></p>
	<?php
	}
}
